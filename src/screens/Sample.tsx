...
const HomeScreen = () => {
    ...
    console.log("ListChild===>", getPredictionListChild);
    ...
    const galleryOpen = async () => {
        try {
            ...
            if (data.status) {
                ...
                setUrl(data?.data)
                await uploadImage(data?.data?.uploadUrl, selectedImage);
            } else {
    ...
    const uploadImage = async (downloadUrl, selectedImage) => {
        try {
    ...
    //         const apiUrl = `${Api}/child/savePrediction`
    //         const body = {
    //             "score": "22.98769760131836",
    //             "label": "SSSSSSSS",
    //             "feeling": "Sad",
    //             "childId": childid,
    ...
    // fetch("https://esra-dev.applab.qa/api/child/savePrediction", requestOptions)
    //     .then((response) => response.text())
    //     .then((result) => console.log(result))
    //     .catch((error) => console.error(error));

    // const savePrediction = async (childid, url, token) => {
    //     const myHeaders = new Headers();
    //     myHeaders.append("Content-Type", "application/json");
    //     myHeaders.append("Authorization", `${token}`);
    ...
    const savePrediction = async () => {
        ...
        try {
            ...
            console.log("savepredictionResponse==>", savepredictionResponse);
